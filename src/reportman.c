...
#include "reportman.h"
int __get_other_pid(int singleton_port, running_pid_t* pid);
...
int d_acquire_singleton(int *sockfd, int singleton_port){
    ...
    if (bind(sockfd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        printf("Bind failed - another instance of reportmand may be running on PID %s: %s", __get_other_pid(singleton_port), strerror(errno) );
...
/// @brief gets the PID if the singleton port is not available. CALLER MUST FREE.
/// @param singleton_port 
/// @param pid 
/// @return number of PIDS in &pid
int __get_other_pid(int singleton_port, running_pid_t* pid){
    FILE *fp;
    ...
    char command[50];
    sprintf(command, "lsof -i :%d", singleton_port);
    ...
    __parse_lsof_output(lsof_output);
    free(lsof_output);
    return num_of_pids;
}
/// @brief Parses output line of LS_OF
/// @param lsof_output 
/// @param pid 
/// @return 
int __parse_lsof_output(const char* lsof_output, running_pid_t* pid){
    char *line;
...
void __free_running_pointers(running_pid_t* pid, unsigned long num_pids){
